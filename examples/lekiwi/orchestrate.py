...
def build_align_prompt(task: str) -> str:
    return (
        ...
        "   Movement rules (scaled with large-swing when far):\n"
        ...
        "     - step_x: if |dx| >= 0.06 → step_x = clip(round(dx*0.35, 3), -0.050, 0.050); else → step_x = clip(round(dx*0.25, 3), -0.020, 0.020).\n"
        "                if abs(step_x) < 0.001 then step_x = sign(dx)*0.001.\n"
        "     - step_y: if |dy| >= 0.08 → step_y = clip(round(dy*0.45, 3), -0.080, 0.080); else → step_y = clip(round(dy*0.25, 3), -0.020, 0.020).\n"
        "                if abs(step_y) < 0.001 then step_y = sign(dy)*0.001.\n"
        "       (round(v,3) = 0.001 resolution; clip caps the maximum swing; farther targets allow larger swings).\n"
        ...
        "     - Write 'reason + action' per axis in clear Japanese. Use only 右/左/前/後ろ and a 3-decimal magnitude.\n"
        "     - For x axis: if detected_x < center_x → '対象が左にあるため、左へ{abs(step_x):.3f}移動する' ;\n"
        "                   if detected_x > center_x → '対象が右にあるため、右へ{abs(step_x):.3f}移動する'.\n"
